...
def train_multi_seed(args, train_df, eval_df, test_df, model_configs):
    ...
    for curr_seed in range(init_seed, init_seed + args.num_of_seeds):
        def objective(trial):
            ...
            num_neurons_per_layer = trial.suggest_int('num_neurons_per_layer', 32, 1024)

            # Define the model configuration based on the hyperparameters
            model_configs["num_hidden_layers"] = num_hidden_layers
            model_configs["num_neurons_per_layer"] = num_neurons_per_layer
            model_configs["learning_rate"] = learning_rate

            # Train the model using the updated configuration
            result = train_multiclass(args, train_df, eval_df, test_df, curr_seed, model_configs)

            # Return the evaluation metric to optimize (e.g., F1 score)
        ...
        # Get the best hyperparameters and update the model configuration
        best_trial = study.best_trial
        best_learning_rate = best_trial.params['learning_rate']
        best_num_hidden_layers = best_trial.params['num_hidden_layers']
        best_num_neurons_per_layer = best_trial.params['num_neurons_per_layer']
        model_configs["learning_rate"] = best_learning_rate
        model_configs["num_hidden_layers"] = best_num_hidden_layers
        model_configs["num_neurons_per_layer"] = best_num_neurons_per_layer

        # Train the model using the best hyperparameters
        result = train_multiclass(args, train_df, eval_df, test_df, curr_seed, model_configs)

        # Keep track of the best result across all seeds
        if best_result is None or result["f1_macro"] > best_result["f1_macro"]:
            best_result = result
            best_result["seed"] = curr_seed
            best_trial = study.best_trial

    # Add the best hyperparameters to the model configuration
    best_learning_rate = best_trial.params['learning_rate']
    best_num_hidden_layers = best_trial.params['num_hidden_layers']
    best_num_neurons_per_layer = best_trial.params['num_neurons_per_layer']
    model_configs["learning_rate"] = best_learning_rate
